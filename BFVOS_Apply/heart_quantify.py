...
def V(minorAxis, majorAxis):
    ...
    # 这个体积模型是第一近似，可以做得更复杂，
    # 一旦完全理解了相机的视角
    volume = 1/6 * np.pi * majorAxis * np.power(minorAxis, 2)
...
def SV(sysV, diaV):
    ...
    stroke_volume = diaV - sysV
    return stroke_volume


def heartrate(fps, sys_0, dia_0, sys_1, dia_1):
    """
    ：参数
        fps:视频的每秒帧数
        sys_frame:找到收缩峰的帧
    ...
    :return:
        每个时间步的估计频率
    """
    sys_heartrate = fps/(1+sys_1-sys_0)
    dia_heartrate = fps/(1+dia_1-dia_0)
    hr = np.mean([sys_heartrate, dia_heartrate])
    return hr
...
def get_ventricular_dimensions(fps, sys, dia, segment_properties):
    ...
    for frame in range(0, len(sys[0])-1):
        ...
        minorFS = FS(ellipse_minor_axis[sys[0][frame]], ellipse_minor_axis[dia[0][frame]])
        majorFS = FS(ellipse_major_axis[sys[0][frame]], ellipse_major_axis[dia[0][frame]])
        sysV = V(ellipse_minor_axis[sys[0][frame]], ellipse_major_axis[sys[0][frame]])
        diaV = V(ellipse_minor_axis[dia[0][frame]], ellipse_major_axis[dia[0][frame]])
        ef = EF(sysV, diaV)
        sv = SV(sysV, diaV)
        hr = heartrate(fps, sys[0][frame], dia[0][frame], sys[0][frame+1], dia[0][frame+1])
