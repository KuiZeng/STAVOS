...
def get_segment_metrics(segment):
    """
    :param segment: cv2 contour
    :return: dictionary with segment properties (area, ellipse axis, equivalent diameter)
    """
...
def segment_analysis(images):
    ...
    for img in images:
        # 分割预测（图像）的阈值化和滤波
        img, seg = filtering(img)
        imgs.append(img)
        segs.append(seg)
        properties.append(get_segment_metrics(seg))

    return properties, imgs, segs
