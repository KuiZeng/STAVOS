...
from data_input import videoLoad, json_to_mask, best_frame_mask_load
...
def main():
    ...
    # 检查是否制作好最佳帧标签
    file_exists = os.path.exists((out_path + "best_frame.json"))
    while not file_exists:
        print(out_path + "best_frame.json", "不存在!!!请制作好最佳帧的标签后，输入yes继续...")
        time.sleep(10)
        while input() != "yes":
            print("输入yes继续")
        file_exists = os.path.exists((out_path + "best_frame.json"))
    ...
    mask = cv2.imread(out_path + "best_frame_mask.png")
    ...
    # #处理《分割结果》-得到《心脏图像量化数据》#
    print("正在进行视频分割分析")
    ventricle_segments = predict_deal(out_path + "predicts/")
    segment_properties, images, segs = segment_analysis(ventricle_segments)  # 调用 image_quantify.py 里的segment_analysis函数 将分割结果进行分割分析，获取椭圆轴、面积、等效直径属性
    ...
    save_electrocardiogram(out_path, seg_area)  # 保存 心电图 # 产生electrocardiogram文件夹
