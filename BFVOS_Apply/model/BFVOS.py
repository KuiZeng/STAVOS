...
class BFVOS(nn.Module):
    ...
    def forward(self, imgs, given_masks, dist, val_frame_ids=None, best_frame_id=0, mode="train"):
        ...
        score_lst = []
        mask_lst = [given_masks[0]]  # 初始化掩码列表为最佳帧的掩码

        # 最佳帧###############################################################################################
        # 提取特征
        with torch.no_grad():
            best_frame_feats = self.vos.encoder(imgs[:, best_frame_id])  # 获取帧序列中最佳帧的特征
        best_frame_key = self.vos.matcher.get_key(best_frame_feats['s16'])  # 获取特征对应的key

        # 为最佳帧的每个对象创建状态字典
        best_frame_state = {}  # 初始化状态字典，键为对象id,值为状态
        for k in object_ids:
            given_seg = torch.cat([given_masks[0] != k, given_masks[0] == k], dim=1).float()  # 将最佳帧掩码转化为二元掩码
            best_frame_state[k] = self.vos.get_init_state(best_frame_key, given_seg)  # 获取初始状态

        # 前续帧###############################################################################################
        state = best_frame_state.copy()
        for i in range(best_frame_id - 1, -1, -1):
            # 提取特征
            with torch.no_grad():
        ...
        # 后续帧###############################################################################################
        state = best_frame_state.copy()
        for i in range(best_frame_id + 1, L):
            # 提取特征
            with torch.no_grad():
                feats = self.vos.encoder(imgs[:, i])  # 获取帧序列中第i帧的特征
            key = self.vos.matcher.get_key(feats['s16'])  # 获取特征对应的key
        ...
        # print("best_frame_id:", best_frame_id)
        if mode == "train":
