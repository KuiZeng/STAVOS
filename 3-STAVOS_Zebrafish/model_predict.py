...
from model.STAVOS import STAVOS
...
def model_predict(model_name, imgs, given_masks, dist, best_frame_id):
    model = STAVOS().eval().cuda()
    model.load_state_dict(torch.load(model_name))

    with torch.no_grad():
        print("正在进行预测")
        vos_out = model(imgs, given_masks, dist.unsqueeze(0), None, best_frame_id, "test")
